...
OS=$(head -n 1 < /etc/issue | cut -d' ' -f1)
UPDATE=""
UPGRADE=""
INSTALL=""
UNINSTALL=""
...
function installExtraSoftware()
{
    ...
    cd /tmp && git clone https://github.com/rofl0r/proxychains-ng.git || exit
    ...
    cd /tmp && git clone https://github.com/noahsai/dde-dock-monito://github.com/noahsai/dde-dock-monitor.git || exit
    cd dde-dock-monitor && qmake && make && sudo cp ./libdde-monitor-plugin.so /usr/lib/dde-dock/plugins || exit
    pkill dde-dock
    echo "  3.3 安装staruml"
...
function configCliSoftware()
{
    echo "5. CLI软件设置"
    echo "  5.1 配置ssh"
    read -p "    如果要使用已有的ssh密钥，请将.ssh文件夹置于${HOME}目录，按任意键继续..." -rn 1
    if [ -d "${HOME}/.ssh" ] && [ -f "${HOME}/.ssh/id_rsa" ]; then
        sudo chmod -R 700 "${HOME}/.ssh"
    else
        read -rp "    输入邮箱：" mail
        ssh-keygen -t rsa -b 4096 -C "${mail}"
        xclip -i -sel clipboard < "${HOME}/.ssh/id_rsa.pub"
    ...
    read -p "按任意键继续..." -rn 1
    cd ${HOME}/.vim/plugged/YouCompleteMe/ || exit && ./install.py --clang-completer
    read -p "请在新打开的终端中进行zsh和tmux(Ctrl-a + Ctrl-I)插件安装，安装完成后输入exit退出返回，按任意键继续..." -rn 1
    zsh
    echo "  5.4 设置privoxy"
...
function configGuiSoftware()
{
    echo "6. 配置GUI软件"
    echo "  6.1 设置开机启动"
    read -p "    请手动配置shadowsocks-qt5,坚果云，XMemo，albert，clipit开机启动，按任意键继续..." -rn 1
    echo "  6.2 登录Chrome"
    read -p "    请手动配置，按任意键继续..." -rn 1
...
configCliSoftware
configGuiSoftware
read -p "配置完成，建议立即重启，按任意键退出..." -rn 1
